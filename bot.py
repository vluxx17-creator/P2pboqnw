...
# ============================================================
# 4. ЛОКАЛИЗАЦИЯ (полная, включая новые ключи)
# ============================================================
LANGUAGES = {
    "ru": {
        "name": "Русский",
    ...
    "en": {
        ...
        "deal_buyer_confirmed": (
            "{check} <b>You confirmed participation in deal #{deal_code}</b>\n\n"
            "<b>Payment address:</b>\n<code>{payment_address}</code>\n\n"
            "<b>Amount to pay:</b>\n{payment_amount}\n\n"
            "<b>Memo:</b>\n<code>{memo}</code>\n\n"
        ...
        "deal_paid_notification": (
            "💰 Buyer @{buyer_username} paid deal #{deal_id}.\n"
            "Complete the deal with command:\n<code>/buyslnft {deal_id}</code>"
        ),
        "deal_completed_buyer": (
            "{check} <b>Deal #{deal_id} successfully completed!</b>\n"
        ...
        "deal_seller_notification": (
            "📢 User @{buyer_username} joined deal #{deal_id}.\n"
            "Successful deals: {buyer_deals}\n\n"
            "⚠️ Make sure it's the same user you've been talking to! "
            "Do not deliver until payment confirmation in this chat!"
        ),
...
async def show_deal_to_buyer(update: Update, context: ContextTypes.DEFAULT_TYPE, deal: dict):
    ...
    seller_username = "makeevdox"  # заменить на реальный username, если есть
...
async def cancel_deal_button(update: Update, context: ContextTypes.DEFAULT_TYPE, deal_id: int):
    query = update.callback_query
    await query.answer()
    user_id = update.effective_user.id
    deal = deals.get(deal_id)
...
# ============================================================
# 11. ДИАЛОГ СОЗДАНИЯ СДЕЛКИ (ИСПРАВЛЕННЫЙ)
# ============================================================
...
# ============================================================
# 12. ДИАЛОГ УПРАВЛЕНИЯ КОШЕЛЬКАМИ
# ============================================================
...
# ============================================================
# 14. ГЛАВНЫЙ ОБРАБОТЧИК КНОПОК (С ОБНОВЛЁННЫМИ ДЕЙСТВИЯМИ)
# ============================================================
